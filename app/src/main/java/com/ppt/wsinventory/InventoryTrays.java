...
public class InventoryTrays extends AppCompatActivity {
    ...
    public static final String TAG = "WS-InventoryTrays";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StateManager.getInstance().setCurrent_activity(InventoryTrays.class.getName());
...
//                e1.setText(myReaderType.toString());
    ...
    private void onReadBarcode(String data) {
        if (!StateManager.getInstance().getCurrent_activity().equalsIgnoreCase(ConfirmPallet.class.getName())) {
//            data = Normalizer.normalize(data, Normalizer.Form.NFD).replaceAll("[^a-zA-Z]", "");
            data = data.replace("\n", "").replace("\r", "");

            BusinessLogic businesslogic = new BusinessLogic(this);
            Inventory_PalletLoc inventory_palletloc = businesslogic.getInventoryPalletLocationByBarcode(data);
            if (inventory_palletloc != null) {
                appContext.setPalletId(inventory_palletloc.getId());
                Intent intent = new Intent(this, ConfirmPallet.class);
